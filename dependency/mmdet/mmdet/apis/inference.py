...
def show_result_ins(img,
    ...
    if cur_result == None:
        ...
        seg_label = cur_result[0] # mask prediction result
        seg_label = seg_label.cpu().numpy().astype(np.uint8)
        cate_label = cur_result[1] # categories predicton result
        cate_label = cate_label.cpu().numpy()
        score = cur_result[2].cpu().numpy()

        vis_inds = score > score_thr # only take the mask and catagory prediction > threshold
        seg_label = seg_label[vis_inds]
        num_mask = seg_label.shape[0]
        cate_label = cate_label[vis_inds]
        cate_score = score[vis_inds]
        ...
        for idx in range(num_mask):
            ...
            center_y, center_x = ndimage.measurements.center_of_mass(cur_mask)
            vis_pos = (max(int(center_x) - 10, 0), int(center_y))
            vis_pos2 = (max(int(center_x) - 10, 0), int(center_y) + 20)
            cv2.putText(img_show, label_text, vis_pos,
                            cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255)) # green
            # cv2.putText(img_show, str(score_m), vis_pos2,
