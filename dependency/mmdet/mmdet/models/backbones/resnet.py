...
from mmdet.models.utils import switchwhiten
import pdb
...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.sw = switchwhiten.SwitchWhiten2d()
...
class ResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 sw_cfg=None,
                 stage_with_sw=(False, False, False, False),
                 norm_eval=True,
                 dcn=None,
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            sw_cfg = self.sw_cfg if self.stage_with_sw[i] else None
            planes = 64 * 2**i
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        if self.deep_stem:
            ...
            self.conv1 = build_conv_layer(
                ...
                bias=False)
            self.norm1_name, norm1 = build_norm_layer(
                self.sw_cfg if self.sw_cfg is not None else self.norm_cfg, stem_channels, postfix=1)
            self.add_module(self.norm1_name, norm1)
            self.relu = nn.ReLU(inplace=True)
