...
int total_frame =0;
int time_count = MIN_MAP_UPDATE_FRAME;
void octo_mapping(){
    while(1){
        if(!odometryBuf.empty() && !pointCloudBuf.empty()){
            ...
            double displacement = std::sqrt((last_pose.translation()-current_pose.translation()).transpose() * (last_pose.translation()-current_pose.translation()));
            Eigen::Vector3d rpy = (last_pose.linear()*current_pose.linear().inverse()).eulerAngles(2, 1, 0);
            double angle_change = std::max(std::max(fabs(rpy.x()),fabs(rpy.y())),fabs(rpy.z()));
            time_count--;
            if(displacement>MIN_MAP_UPDATE_DISPLACEMENT||time_count<0 || angle_change>MIN_MAP_UPDATE_ANGLE){
                start = std::chrono::system_clock::now();
                octoMapping.updateLocalMap(pointcloud_in,current_pose);
                end = std::chrono::system_clock::now();
                std::chrono::duration<float> elapsed_seconds = end - start;
                ROS_INFO("Map update, time cost %f ", elapsed_seconds.count() * 1000);
                octomap::ColorOcTree* octo_map = octoMapping.getMap();
                last_pose = current_pose;
                time_count = MIN_MAP_UPDATE_FRAME;
                // publish octomap
